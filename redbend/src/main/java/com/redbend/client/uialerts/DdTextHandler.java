 ...
 * DdTextHandler.java
 *
 * Parses the description field of a Download Descriptor. The default
 * description is defined in DMA_VAR_FUMO_DP_DESCRIPTION.
 *
...
public class DdTextHandler {
	...
	public static String [] getDdEnvTypeArr(Event ev) {
		byte envTypeByte [] =  ev.getVarStrValue("DMA_VAR_DP_ENV_TYPE");
		String envTypeArr [] = null;
		if (envTypeByte != null)
		{
			String envType = new String(envTypeByte);
			envTypeArr = envType.split(String.valueOf(DELIMITER), -1);
		}
		return envTypeArr;
	}
	...
	public static String getDdInfoUrl(Event ev) {
		...
		Log.d(LOG_TAG, "+getDdInfoUrl: infoUrlValue: " + infoUrlValue);
		String infoUrlArr [] = infoUrlValue.split(String.valueOf(DELIMITER), -1);
		for(String str :infoUrlArr) {
			urlBuffer = urlBuffer.append(str).append(TYPE_HTML_AND_LOCALE)
					.append(getLocale()).append("\n");
		    }    
