 ...
 *
 * Displays an installation complete notification. The installation result is
 * defined in DMA_VAR_SCOMO_RESULT.
 * 
 * Receives DIL events:
 * DMA_MSG_SCOMO_INSTALL_DONE (background only)
 *
 ...
 * defined in DMA_VAR_REPORT_RESULT. 
 */
public class InstallDoneNotificationHandler extends NotificationHandlerBase {
	...
	private void IconsResourcesInit(Event ev) {
		...
		if (m_isInstall){
			...
			if (m_result == SCOMO_SUCCESS || m_result == FUMO_SUCCESS) {
				...
				m_smallIconResId = R.drawable.ic_notify_success;
			} else { // failed to install
				m_smallIconResId = R.drawable.ic_notify_failure; 
			}
		} else {// remove
			m_smallIconResId = R.drawable.ic_notify_remove;
		}
	}
	...
	protected int initiate(Event ev) {
		...
		m_result = ev.getVarValue(DMA_VAR_REPORT_RESULT);
