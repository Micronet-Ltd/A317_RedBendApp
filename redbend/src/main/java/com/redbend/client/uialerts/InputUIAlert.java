 ...
 * InputUIAlert.java
 *
 * Displays a text entry message screen. The screen displays an input field
 * prefilled with the text value DMA_VAR_UI_ALERT_TEXT and closes when the
 * end-user taps OK or after DMA_VAR_UI_ALERT_MAXDT seconds have passed. Used
 * during self-registration
 * 
 * Receives DIL events:
 * DMA_MSG_UI_ALERT if DMA_VAR_UI_ALERT_TYPE=DMA_UI_ALERT_TYPE_INPUT (foreground only)
 *
 * Sends BL events:
