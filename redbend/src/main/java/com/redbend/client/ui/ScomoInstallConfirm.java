 ...
 *
 * Prompts the end-user to confirm or reject the installation.
 *
 * Receives DIL events:
 * DMA_MSG_SCOMO_INS_CONFIRM_UI (not silent update only)
 * DMA_MSG_SCOMO_INS_CONFIRM_UI if DMA_VAR_DEVINIT_SESS=1 (not silent update only, foreground only)
 * 
 * Sends BL events:
 * DMA_MSG_SCOMO_ACCEPT
 * DMA_MSG_SCOMO_POSTPONE
 * DMA_MSG_SCOMO_CANCEL
 *
