 ...
 *******************************************************************************
 */
typedef VDM_Error (*VDM_BootGetAddrTypeCB)(UTF8CStr inAddr, UTF8CStr *outAddrType, void* inContext);
 ...
 *
 * \note	The \a ioContext is de-allocated by the vDirect Mobile Engine after
...
typedef struct 
{
	IU32	size;							/*!< The number of bytes to be
											     downloaded from the URI */
	UTF8Str	strField[E_VDM_DDField_count];	/*!< String values (Note:
 ...
 * \param	inContext				Context passed in \ref VDM_triggerDLSession.
 * 
 * \return	TRUE if a synchronous confirmation and vDirect Mobile should start
 *			downloading the package immediately, FALSE for an asynchronous
 *			operation. If FALSE, vDirect Mobile will wait for an confirmation or
...
/*!
 * Pointer to a list of alerts
 */
typedef VDM_GenAlertList_Node_t* VDM_GenAlertListPtr;
 ...
 * is allocated in this API and is later released by vDirect Mobile.
 *
...
typedef VDM_Error  (*VDM_encryptCB)(
	void *inContext,
	UTF8CStr	inPlainText,
	IU32	inPlainTextLen,
	UTF8Str	outCipherText,
	IU32*	ioCipherLen,
	IBOOL   inIsLast);
...
typedef VDM_Error  (*VDM_decryptCB)(
	void *inContext,
	UTF8CStr inCipherText,
	IU32	inCipherTextLen,
	UTF8Str	outPlainText,
	IU32* ioPlainTextLen,
	IBOOL inIsLast);
