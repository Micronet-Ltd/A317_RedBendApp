...
public class FlowManager {
	...
	private int uiMode = SmmService.UI_MODE_FOREGROUND;
	...
	private Notification getBackgroundNotif() {
		return (backgroundNotif != null) ? backgroundNotif.getNotification() : null;
	}
	...
	public void handle(EventHandler handler, Event ev, int ui) {
		...
		if (handler.hasActivity()) {
			FlowUtils.dLog(LOG_TAG, m_flowId, "handle::handler.hasActivity()");
			Intent i = handler.getIntent();
			
			if ((ui & getUiMode()) == 0 && !isSameComponent(i)) {
			...
			if (n != null && (ui & getUiMode()) != 0)
				notificationManager.notify(BACKGROUND_NOTIF_ID + m_flowId, n.getNotification());
		}
		FlowUtils.dLog(LOG_TAG, m_flowId, "-handle");
	}
